 ...
 *
 * If the macro _SYSTEM is defined, then _SIGN is set to negate its argument,
 * otherwise it returns its argument unchanged. Thus when compiling the
 * operating system, the macro _SYSTEM will be defined, setting EPERM to (-1),
 * whereas when this file is included in an ordinary user program, EPERM has
 * the value (1).
 */
...
/* Now define _SIGN as a function-like macro depending on _SYSTEM. */
#ifdef _SYSTEM
# define _SIGN(n) (-(n))
# define OK 0
#else
...
/* Avoid conflict with standard library errno */
#ifndef _KERNEL
